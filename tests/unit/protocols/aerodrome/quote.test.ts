...
import { quoteExactInputSingle } from "../../../../src/protocols/aerodrome/quote.js";
...
function buildCtx(opts?: { quoter?: Address }): {
  ...
  const ctx = {
    ...
    addresses: {
      weth: TOKEN_IN,
      aerodrome: { npm: "0x0" as Address, quoter: opts?.quoter },
    },
...
describe("quoteExactInputSingle (aerodrome)", () => {
  it("retorna o amountOut efetivo cotado pelo Quoter", async () => {
  ...
  it("chama o contrato Quoter com tickSpacing e sem price limit", async () => {
    ...
    expect(arg.tickSpacing).toBe(200);
  });
