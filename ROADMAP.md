...
6. **Known issues do SECURITY.md.** R-03 (retry sem backoff), T-04 (slippage não valida upper bound), L-03 (ausência de deadline default nas txs de escrita). Task 19 registrou sem fix.
...
3. **Subpath por protocolo em packages separados.** Quebrar em `@fsa-tools/web3-core` + `@fsa-tools/web3-uniswap-v3` + `@fsa-tools/web3-aave` + `@fsa-tools/web3-aerodrome`. Tree-shaking mais agressivo, deps opcionais por bot.
