...
import { AERODROME_QUOTER_ABI } from "../../abis/aerodrome-quoter.js";
...
/** Aerodrome Quoter não aplica price limit — quote sobre o swap completo. */
const NO_PRICE_LIMIT = 0n;

/**
 * Cotação real de um swap single-hop via Aerodrome Slipstream Quoter.
 *
 * Diferente de `spotAmountOut` (estimativa marginal, sem price impact),
 * o Quoter simula o swap completo on-chain e retorna o `amountOut`
...
export async function quoteExactInputSingle(
  ...
  const quoter = ctx.addresses.aerodrome?.quoter;
  if (!quoter) {
